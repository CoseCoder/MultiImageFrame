...
namespace blend {
	...
	cv::Mat buildPixelToIndexLookup(cv::Mat mask, int &npixel)
	{
		cv::Mat_<uchar> m = makeContinuous(mask);
		...
		cv::Mat result_)
	{
		...
		cv::Mat f = makeContinuous(f_);
		cv::Mat_<uchar> bm = makeContinuous(bdMask_);
		cv::Mat bv = makeContinuous(bdValues_);
